...
// Any subsequent call to this function may reset the timer
...
//Context to handle exiting threads
...
void thread_exit_handler(){
    ...
    //Will allow mythreads_join to work, and handler to remove it from the thread queue
    curr->state = FINISHED; 
...
//Initializes context for exiting threads
...
int mythreads_create(mythr_t *thr, void (body)(void *), void *arg){
    ...
    if(getcontext(&thr->context)) return -1;
...
int mythreads_sleep(int secs){
    //continuously yield while not enough time has passed
